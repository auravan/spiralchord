...
import argparse
import time # For tracking animation progress
...
class InteractiveSpiralPiano:
    def __init__(self, n_rotations=5, enable_animation=True):
        ...
        # SINGLE ANIMATION LOOP MANAGEMENT
        # Stores tuples: (note_key, start_time, duration, decay_end_factor, line_obj)
        self.active_decay_animations = [] # List of notes currently decaying
        self.master_animation = None # The single FuncAnimation object
        ...
        # Start the master animation loop only once
    ...
    def _draw_notes_segments(self):
        for octave_idx in range(self.n_rotations):
            for semitone_idx in range(12):
                ...
                # Reduced points for potential performance gain
                num_points_in_segment = 30 # Was 50
    ...
    def _on_key_press(self, event):
        if event.key == ' ': # Spacebar for batch decay when animation is OFF
            ...
            if self.enable_animation and self.active_notes_list:
                ...
                self._trigger_batch_decay(force_animation=True) # Force animations to run
    ...
    def _start_master_animation_loop(self):
        # This function sets up the ONE master animation loop
        ...
        # We don't store individual anims anymore, just this master one.

    def _update_decay_animations(self, frame):
        # This is the single animation function that updates ALL decaying notes
        ...
        for decay_info in list(self.active_decay_animations):
            ...
            if progress < 1.0:
                # Calculate current r and alpha based on progress
                # Note: original_r_data and original_theta_data are stored in notes_data
                original_r_data = self.notes_data[note_key]['base_r']
                original_theta_data = self.notes_data[note_key]['base_theta']

                current_r_factor = 1.0 - progress * (1.0 - decay_end_factor) # Linear interpolation for r factor
                current_alpha = 1.0 - progress # Linear decay for alpha
                ...
                line_obj.set_alpha(0.0) # Ensure it's invisible
                notes_to_remove.append(decay_info)
                
                # Update state and display if it was still considered highlighted
                if note_key in self.notes_data and self.notes_data[note_key]['highlighted']:
                    ...
                    if note_name in self.active_notes_list: # This might already be cleared by batch decay
                        self.active_notes_list.remove(note_name)
                    self._update_status_display()
        
        # Remove finished animations from the list
        for note_info in notes_to_remove:
            if note_info in self.active_decay_animations: # Check before removing
                self.active_decay_animations.remove(note_info)

        return lines_to_redraw # Return the list of artists that were modified

    def _add_note_to_decay_queue(self, note_key):
        # Ensure only one decay animation is queued per note
        self._cancel_decay_animation_for_note(note_key) 
        
        duration = 0.5 # Same animation duration as before
    ...
    def _cancel_decay_animation_for_note(self, target_note_key):
        # Removes a note from the active_decay_animations list
        for decay_info in list(self.active_decay_animations): # Iterate over a copy
            ...
            if note_key == target_note_key:
                # Ensure the line object is immediately reset to invisible
                line_obj.set_alpha(0.0)
    ...
    def _trigger_batch_decay(self, force_animation=False):
        ...
        # Clear the active notes list and update status BEFORE starting animations
        ...
        for note_key in notes_to_decay_keys:
            ...
            line_obj.set_data(base_theta, base_r) # Reset to full length
            line_obj.set_alpha(1.0) # Make fully visible
