...
class InteractiveSpiralPiano:
    def __init__(self, n_rotations=5, enable_animation=True):
        ...
        # Make this attribute directly controllable
        ...
        self._connect_events() # Connect events after all components are ready
    ...
    def _update_title(self):
        # Helper to update title based on animation state
    ...
    #         self._update_title() # Update title to reflect new state
    #         print(f"Animation is now {'ON' if self.enable_animation else 'OFF'}")
    #         # If animation is turned ON, and there are active notes, clear them
    #         # to prevent weird states. Or, trigger their decay immediately.
    ...
    #             self._trigger_batch_decay(force_animation=True) # Force animation even if it was off
    def _on_key_press(self, event):
            if event.key == ' ': # Spacebar for batch decay when animation is OFF
                if not self.enable_animation:
                    self._trigger_batch_decay()
        ...
        for note_key, data in self.notes_data.items():
            ...
            if contains:
                ...
                if not data['highlighted']: # If currently not highlighted (or animation just stopped)
                    ...
                    # This branch is primarily for when animation is OFF, allowing notes to be "unclicked"
    ...
    def _start_decay_animation(self, note_key, line_obj, original_theta_data, original_r_data):
        # Check this setting here as well, to prevent starting animations if globally off
        if not self.enable_animation:
            return
        ...
        def animate_frame(frame_idx):
            if frame_idx < n_frames:
                ...
                if note_key in self.notes_data: # Check if key still exists
                    # Only reset highlighted state if it's still marked as highlighted
                    # This prevents conflicts if the user clicked it off during animation
    ...
    # Added force_animation parameter for _trigger_batch_decay
    def _trigger_batch_decay(self, force_animation=False):
        """Triggers decay animation for all currently active notes.
        If force_animation is True, it will run even if enable_animation is False."""
        if not self.enable_animation and not force_animation: # Only run if animation is generally off, or if forced
            return
        ...
        for note_name_to_decay in notes_to_decay:
            ...
            if note_key_found: # No need to check 'highlighted' here, as we iterate active_notes_list
                ...
                line_obj.set_alpha(1.0) # Always ensure it's visible before animating out

                # Temporarily enable animation for this specific decay if needed
                original_animation_state = self.enable_animation
                self.enable_animation = True # Force animation for this call
                self._start_decay_animation(note_key_found, line_obj, base_theta, base_r)
                self.enable_animation = original_animation_state # Restore original state
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable-animation",
        action="store_true", # If this flag is present, enable_animation will be True
